...
const startSession = async (result: AuthResult) => {
  if (result.user.role !== 'delivery_partner') {
    throw new ApiError(
      'WRONG_APP',
      'This app is for delivery partners. Customers can order in the MealDirect app.',
      403
    );
  }
...
export const register = createAsyncThunk(
  ...
  async (
    ...
    try {
      return await startSession(await authApi.register({ ...input, role: 'delivery_partner' }));
    } catch (error) {
      await session.clearSession();
      return rejectWithValue(rejectMessage(error));
    }
