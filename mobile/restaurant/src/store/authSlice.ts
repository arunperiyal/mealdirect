...
const startSession = async (result: AuthResult) => {
  if (result.user.role !== 'restaurant_admin') {
    throw new ApiError(
      'WRONG_APP',
      'This app is for restaurant partners. Customers can order in the MealDirect app.',
      403
    );
  }
...
export const register = createAsyncThunk(
  ...
  async (
    ...
    try {
      return await startSession(await authApi.register({ ...input, role: 'restaurant_admin' }));
    } catch (error) {
      await session.clearSession();
      return rejectWithValue(rejectMessage(error));
    }
